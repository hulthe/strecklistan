...
pub fn get_signup(signup_id: i32) -> Result<Json<Signup>, ErrorJson>{
    ...
    let result: Signup = event_signups.find(signup_id)
        .first(&connection)?;
    Ok(Json(result))
}
...
pub fn get_event_signups(event_id: i32) -> Result<Json<Vec<Signup>>, ErrorJson>{
    use schema::tables::event_signups::dsl::*;
    let connection = establish_connection()?;
    let result: Vec<Signup> = event_signups
        ...
        for new_event in generate_new_events(5, 5) {
            ...
            for new_signup in signups {
                let mut response = client.post("/signup")
                    .body(serde_json::to_string(&new_signup)
                        .expect("Could not serialize NewSignup"))
                ...
                let signup: Signup = serde_json::from_str(&body)
                    .expect("Could not deserialize JSON into Signup");
                assert_eq!(signup.name, new_signup.name);
                assert_eq!(signup.email, new_signup.email);
                assert_eq!(signup.event, new_signup.event);

                let mut response = client.get(format!("/signup/{}", signup.id))
                    .dispatch();
                let body = response.body_string().expect("Response has no body");
                let signup2: Signup = serde_json::from_str(&body)
                    .expect("Could not deserialize JSON into Signup");
