...
#[get("/events?<low>&<high>")]
pub fn get_event_range(low: i64, high: i64, db_pool: State<DatabasePool>) -> Result<Json<Vec<EventWS>>, SJ> {
    Ok(Json(get_event_ws_range(db_pool.inner().get()?, low, high, true)?))
}